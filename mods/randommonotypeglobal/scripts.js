exports.BattleScripts = {
  ...
  randomMonoTeam: function(side) {
		...
		var seed = Number(side.battle.id.match(	/\d+$/));
		seed = seed * 137;
		var exlength = 0;
		for (var i=0; i<5; i++) {
			seed = Math.pow(seed,2)
			seed = (seed - seed%100)/100;
			exlength = seed.toString().length-3;
			if (exlength>0) seed = (seed - seed%Math.pow(10,exlength))/Math.pow(10,exlength);
		}
		var sidetype = typesarray[seed%17];

exports.BattleScripts = {
  ...
  randomUbersTeam: function(side) {
		var keys = [];
		var pokemonLeft = 0;
		var pokemon = [];
		for (var i in this.data.FormatsData) {
			if (this.data.FormatsData[i].viableMoves && this.data.FormatsData[i].tier === "Uber") {
				keys.push(i);
			}
		}
		keys = keys.randomize();
		...
		for (var i=0; i<keys.length && pokemonLeft < 6; i++) {
			...
			var set = this.randomUbersSet(template, i);
	...
	randomUbersSet: function(template, i) {
		if (i === undefined) i = 1;

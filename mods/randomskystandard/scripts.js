exports.BattleScripts = {
  ...
  randomSkyTeam: function(side) {
		...
		var mustmove = "fly"
                              	//var banarray = [487,492,493]; //Giratina,Shaymin,Arceus
	...
	var poke = formes.sample();
                var template = this.getTemplate(poke);
  	var set = this.randomSkySet(template, i);
	team.push(set);
	}
                //console.log(team);
                return team;
	...
	randomSkySet: function(template, i) {
		var musttype = "Flying";
		var mustability = "Levitate";
		var mustmove = "fly";
		if (i === undefined) i = 1;
			...
			if (!template) {
				...
				if (!righttype) {
					if (template.abilities['0'] === mustability || template.abilities['1'] === mustability || template.abilities['DW'] === mustability) {
						...
						console.log("Debug: case \'else\' acceded in randomSkySet. Contact the developer of the tier.");
					}
				}
				else {

exports.BattleScripts = {
  ...
  randomSkyBalloonTeam: function(side) {
               		var teamdexno = [];
                	var team = [];
		var musttype = "Flying"
		var mustability = "Levitate"
		var mustmove = "fly"
		var mustitem = "Air Balloon"
		var countitem = 0
                  	//var banarray = [487,492,493];
		//Giratina,Shaymin,Arceus
           		...
           		//pick six random pokemon--no repeats, even among formes
					...
					for (var j=0; j<formes.length; j++) {
                                        				...
                                        				if (!type2) {
                                        					...
                                        					var righttype = (type1==musttype);
                                        				}
                                        				else { 
                                        					//console.log(i + ", " + x + ", " + template.species + ", " + type1 + " and " + type2);
                                        					var righttype = (type1==musttype || type2==musttype);
                                        				}
						var rightability = (template.abilities['0'] == mustability || template.abilities['1'] == mustability || template.abilities['DW'] == mustability);
						if (typeof(template.learnset)!="object") {
							var rightmove = false;
						} else {
							var rightmove = Object.keys(template.learnset).toString().split(",").indexOf(mustmove) != -1;
						}
						var preallowed = (righttype || rightability || rightmove);
						if (preallowed) break;
					}
					if (!preallowed && !template.requiredItem && countitem == 0 && i !=0) {
						//first poke will never have Air Balloon
						countitem++;
						var rightitem = true;
					}
					else {
						var rightitem = false;
                                		...
                                		while (!(righttype || rightability || rightmove || rightitem));
	...
	for (var j in this.data.Pokedex) {
		...
		if (this.data.Pokedex[j].num === teamdexno[i] && this.getTemplate(this.data.Pokedex[j].species).learnset) {
			var righttype = prototemplate.types[0] == musttype || prototemplate.types[1] ==musttype;
			var rightability = prototemplate.abilities['0'] == mustability || prototemplate.abilities['1'] == mustability || prototemplate.abilities['DW'] == mustability;
			var rightmove = Object.keys(prototemplate.learnset).toString().split(",").indexOf(mustmove) != -1;
			var skyallowed = righttype || rightability || rightmove;
			if(skyallowed) formes.push(this.data.Pokedex[j].species);
                                }
	}
	if (formes.length == 0) {
		for (var j in this.data.Pokedex) {
	...
	var poke = formes.sample();
                var template = this.getTemplate(poke);
  	var set = this.randomSkyBalloonSet(template, i);
	team.push(set);
	}
                //console.log(team);
                return team;
	...
	randomSkyBalloonSet: function(template, i) {
		var musttype = "Flying";
		var mustability = "Levitate";
		var mustmove = "fly";
		var mustitem = "Air Balloon";
		...
		//moves[0] = 'Safeguard';
		{
			...
			//here comes my edition: after the assignment of moves and abilities so that I make sure that it isn't overriden
			if (!template) {
				console.log("Debug: template " + i + " was undefined.");
			}
			else {	
				if (!template.types[1]) {
					//console.log(i + ", " + template.species + ", only " + template.types[0]);
					var righttype = template.types[0] == musttype
				}
				else {
					//console.log(i + ", " + template.species + ", " + template.types[0] + " and " + template.types[1]);
					var righttype = template.types[0] == musttype || template.types[1] == musttype
				}
				if (!righttype) {
					if (template.abilities['0'] === mustability || template.abilities['1'] === mustability || template.abilities['DW'] === mustability) {
						ability = mustability;
					}
					else if (Object.keys(template.learnset).toString().split(",").indexOf(mustmove) != -1) {
						moves[3] = "fly";
					}
					else {
						var needsballoon = true;
					}
				}
				else {
					//console.log(template.name + " was found to be of the right type")
				}
			}
			if (hasMove['gyroball']) {
				ivs.spe = 0;
